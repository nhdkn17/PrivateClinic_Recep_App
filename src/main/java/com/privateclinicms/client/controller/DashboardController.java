...
import com.privateclinicms.client.controller.medicalThread.AddPatientController;
import com.privateclinicms.client.controller.medicalThread.PatientThreadListener;
import com.privateclinicms.client.controller.other.Dialog;
import com.privateclinicms.client.service.CL_AppointmentService;
import com.privateclinicms.client.service.CL_DashboardService_Recep;
...
public class DashboardController implements PatientThreadListener {
    ...
    private CL_DashboardService_Recep dashboardService;
    private CL_AppointmentService CLAppointmentService;
    private final Map<Integer, Thread> patientThreads = new HashMap<>();
    private CL_PatientService CLPatientService;
    private BenhNhanDAO benhNhanDAO;
    private ThuocDAO thuocDAO;
    private XMLExporter exporter;

    public void initialize() {
        dashboardService = new CL_DashboardService_Recep();
        CLAppointmentService = new CL_AppointmentService();
        CLPatientService = new CL_PatientService();
...
//                    showPatientDetail(maBenhNhan);
    ...
    public void loadDashboardData() {
        try {
            ...
            String doanhThu = dashboardService.getDoanhThuHomNay();
    ...
    private void loadAppointmentsToday() {
        try {
            List<LichKhamModel> appointments = CLAppointmentService.getAppointmentsByDate(LocalDate.now());
    ...
    private void loadAppointmentsByDate(LocalDate selectedDate) {
        try {
            List<LichKhamModel> appointments = CLAppointmentService.getAppointmentsByDate(selectedDate);
    ...
    private void handleXemDoanhThuTheoThang() {
        ...
        if (thang != null && nam != null) {
            try {
                Map<Integer, BigDecimal> map = dashboardService.getDoanhThuTheoNgay(thang, nam);
                ...
                e.printStackTrace();
                Dialog.showNotice("Lỗi", "Không thể tải dữ liệu doanh thu", false);
            }
        }
    }
...
//        try {
//            BenhNhan bn = benhNhanDAO.getPatientById(maBenhNhan);
//            if (bn != null) {
