...
import com.privateclinicms.shared.dao.PhieuKhamDAO;
...
public class PatientController {
    ...
    private final CL_PatientService CLPatientService = new CL_PatientService();
    ...
    private void loadBenhNhanData() throws IOException {
        benhNhanList = FXCollections.observableArrayList(CLPatientService.getAllPatients());
    ...
    private void handleAdd() {
        try {
            ...
            int maBenhNhanMoi = CLPatientService.insertAndReturnId(benhNhan);
            if (maBenhNhanMoi > 0) {
                PhieuKham phieu = new PhieuKham();
                phieu.setMaBenhNhan(maBenhNhanMoi);
                phieu.setMaBacSi(1);
                phieu.setNgayKham(Date.valueOf(LocalDate.now()));
                phieu.setTrieuChung("Chưa có");
                phieu.setChanDoan("Chưa có");
                phieu.setTrangThai("Chưa khám");

                new PhieuKhamDAO().insert(phieu);
            }

            Dialog.showNotice("Thành Công", "Đã thêm bệnh nhân và phiếu khám!", true);
            loadBenhNhanData();
            clearForm();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void handleUpdate() {
        try {
            if (selectedBenhNhan != null) {
                ...
                CLPatientService.updatePatient(selectedBenhNhan);
    ...
    private void handleDelete() {
        try {
            if (selectedBenhNhan != null) {
                CLPatientService.deletePatient(selectedBenhNhan.getMaBenhNhan());
